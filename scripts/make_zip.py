...
def  main():
  ...
  installer = "\installer\RNNoise Installer.exe"
   
  if demo:
    installer = "\installer\RNNoise Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\RNNoise manual.pdf" 
  ]
  ...
  ZIPNAME = "RNNoise-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "RNNoise-v" + FULLVERSIONSTR + "-win-demo.zip"
